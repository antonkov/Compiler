...
/*
  override def token: Parser[Token] =
    ( string ^^ { case s: String => StringLit(s) }
  ...
  def zero: Parser[String] = '0' ^^ { case _ => "0" }
  def nonzero = elem("nonzero digit", d => d.isDigit && d != '0')
  def exponent: Parser[String] = elem("exponent character", d => d == 'e' || d == 'E') ^^ { case s => s. mkString "" }
  def sign = elem("sign character", d => d == '-' || d == '+')

  def v: Parser[String] = '\\' ~ 'u' ^^ { case s:String => s}
  ...
  def charSeq: Parser[String] =
    ('\\' ~ '\"' ^^ { case _ => "\"" }
      ...
      |'\\' ~ 'r'  ^^ { case _ => "\r" }
      |'\\' ~ 't'  ^^ { case _ => "\t" }
      |'\\' ~ 'u' ~ unicodeBlock ^^ { case a ~ b => a + b})

  val hexDigits = Set[Char]() ++ "0123456789abcdefABCDEF".toArray
  def hexDigit = elem("hex digit", hexDigits.contains(_))

  private def unicodeBlock: Parser[String] = hexDigit ~ hexDigit ~ hexDigit ~ hexDigit ^^ {
    case a ~ b ~ c ~ d => new String("aba")
